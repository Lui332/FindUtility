 ...
 *                 ./a.out -i 12394131
 *                 ./a.out -n exampleFile 
 * OUTPUT: Prints all file paths within directory provided that satisfy criteria
 */
...
void read_dir(char* sub_dir, char* name, char* mmin, char* inum) {
    ...
    if(sub_dp!=NULL) {                                //if directory opened successfully
                ...
                if(subsubdp!=NULL) {                                  //if directory opened successfully
                    ...
                    if (strcmp(name, "EMPTY") != 0) {                       //if name of dir needs to be checked
                        if (strcmp(name, sub_dirp->d_name) != 0) {          //check if names match
                            read_dir(temp_full_path, name, mmin, inum);         // if no match, continue into sub-dir
                            continue;                                       
                        }
                    }
                    ...
                    if (stat(temp_full_path, &buf) == 0) {
                        ...
                        if (strcmp(mmin, "EMPTY") != 0) {                                       //if mmin of dir needs to be checked   
                            ...
                            if (mmin[0] != '-' && mmin[0] != '+' && desired_minute_range != last_modified_minutes) {   //if times do not match
                                read_dir(temp_full_path, name, mmin, inum);                                              //continue into sub-dir
                                continue;
                            }
        ...
        closedir(sub_dp);    //close opened directory
...
int main(int argc, char **argv) {
    int w;
    char *where = ".", *name = "EMPTY", *mmin = "EMPTY", *inum = "EMPTY";   //set default criteria

    if(argc < 2) {                                       //if less than two command line argruments
        read_dir(where, "EMPTY", "EMPTY", "EMPTY");          //read current dir and exit
        exit(0);
    } else {
    ...
    read_dir(where, name, mmin, inum);     //read directory using criteria given
